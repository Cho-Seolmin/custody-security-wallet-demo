...
웹 UI에서는 SSS **샤드 복원** 모달에 문서의 **레거시 hex 샤드 3개**를 import하세요 (오프라인 `recover`로 나온 private key를 붙여 넣는 입력란은 없습니다).  
키·샤드는 브라우저 서명에만 사용되며 **서버로 전송되지 않습니다.** 요청 성공 후 프론트 복원 세션은 비워집니다.
