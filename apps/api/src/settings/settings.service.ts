...
export class SettingsService {
  ...
  async updatePreferences(userId: string, dto: UpdatePreferencesDto) {
    if (dto.defaultWalletId) {
      const wallet = await this.prisma.wallet.findUnique({ where: { id: dto.defaultWalletId } });
    ...
    return this.prisma.userPreference.upsert({
      where: { userId },
      update: {
        ...(dto.defaultWalletId !== undefined ? { defaultWalletId: dto.defaultWalletId } : {}),
        ...(dto.balanceUnit !== undefined ? { balanceUnit: dto.balanceUnit } : {}),
        ...(dto.autoRefreshEnabled !== undefined ? { autoRefreshEnabled: dto.autoRefreshEnabled } : {}),
