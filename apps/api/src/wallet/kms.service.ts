    ...
    self
      ...
      .obj(
        self
          .key('algorithm')
          .seq()
          .obj(self.key('algorithm').objid(), self.key('parameters').objid()),
        self.key('publicKey').bitstr(),
      );
  },
);

const EcdsaDerSignature = asn1.define(
  'EcdsaDerSignature',
  function (this: any) {
    const self = this;
    self.seq().obj(self.key('r').int(), self.key('s').int());
  },
);
...
export class KmsService {
  ...
  private static readonly KMS_ADVISORY_LOCK_KEY = 7483621n;
  ...
  private parseDerSignature(derSignature: Uint8Array): {
    r: bigint;
    s: bigint;
  } {
  ...
  async getAddress(): Promise<string> {
    ...
    const decoded = SubjectPublicKeyInfo.decode(
  ...
  private async withKmsSendLock<T>(fn: () => Promise<T>): Promise<T> {
    const lockRows = await this.prisma.$queryRaw<{ acquired: boolean }[]>`
      SELECT pg_try_advisory_lock(${KmsService.KMS_ADVISORY_LOCK_KEY}) AS acquired
    `;
    ...
    try {
      ...
      await this.prisma.$queryRaw`
        SELECT pg_advisory_unlock(${KmsService.KMS_ADVISORY_LOCK_KEY})
      `;
    }
  }
  ...
  async sendNativeTransaction(
    ...
    return this.withKmsSendLock(async () => {
      ...
      this.logger.error(
        `KMS sendNativeTransaction failed: ${error?.message || error}`,
      );
      ...
      if (message.includes('nonce') || message.includes('KMS_NONCE_ERROR')) {
        throw new Error(
